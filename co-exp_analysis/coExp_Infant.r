...
# **********load Counts file***********   #
counts_data <- read.csv(file = "Counts_Infant.csv", header = TRUE, sep = ",")
...
dds75<- dds[rowSums(counts(dds) >= 25) >= 13]
nrow(dds75) #14480 
...
###   **********Save the plot**************   ###
pdf("dendrogram_plot_norm_Infant_new.pdf", width = 12, height = 8)
par(mar = c(5, 4, 4, 2) + 0.1)
plot(dend_norm, main = "Hierarchical Clustering of Infant Samples")
dev.off()
...
#-----------------------------------------------------------------------------------------------------------------
# to plot the module membership(MM) vs gene trait significance (GS) of turquoise module 
#------------------------------------------------------------------------------------------------------------------
...
# Creating a data frame for the genes
...
filtered_turquoise_edges_0.45 <- filtered_turquoise_edges[filtered_turquoise_edges$weight >= 0.2, ] # Filter edges with weight >= 0.2
...
#----------------------------------------------------------------------------------------------------
# network of selected genes
#---------------------------------------------------------------------------------------------------------

# filtered_turquoise_edges is the edge file from turquoise module that contain only the selected hub genes 
...
plot(g, 
     ...
     main = "Network in Infant Samples")  


# saving ***********
save.image(file = "Infant.RData")
