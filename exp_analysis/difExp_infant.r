...
# Read the file
counts.data <- read.csv("Counts_Infants.csv", header = TRUE)
view(counts.data)
...
res
summary(res)
#out of 48446 with nonzero total read count
#adjusted p-value < 0.1
#LFC > 0 (up)       : 8150, 17%
#LFC < 0 (down)     : 9047, 19%
#outliers [1]       : 0, 0%
#low counts [2]     : 15028, 31%
#(mean count < 1)
...
sigs
summary(sigs)
#out of 15433 with nonzero total read count
#adjusted p-value < 0.1
#LFC > 0 (up)       : 7274, 47%
#LFC < 0 (down)     : 8159, 53%
#outliers [1]       : 0, 0%
...
ggplot(THAPCounts, aes(x=cell_type, y=count, color=cell_type, group=cell_type)) + 
  ...
  ggtitle("THAP9 Counts by Cell Type Infant samples") +          
  ylab("Counts") +                                   
  xlab("Cell Type")                                 
...
ggplot(df_thap9, aes(x = cell_type, y = Expression, fill = cell_type)) +
  geom_boxplot() +
  labs(title = "Expression of THAP9 in MOs and OPCs Infant samples",
...
volcano_infant <- ggplot(sigs) +
  ...
  labs(title = "Volcano plot",
       subtitle = "MOs v/s OPCs in Infant samples") +
  theme_bw() +
...
ggplot(df_thap9, aes(x = Condition, y = Expression, fill = Condition)) +
  ...
  labs(
    title = "Expression of THAP9 in MOs vs OPCs (Infants)",
...
grid.text("Infant Samples (MOs vs OPCs)", 
